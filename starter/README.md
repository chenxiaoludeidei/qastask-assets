...
你需要在固定入口文件 `main.py` 中实现一套抗噪量子架构搜索（NA-QAS）方法。当前工作目录已提供starter'main.py'。请在该文件基础上修改，不要删除或重建工程结构。出题方已经提供了可运行的基础框架、公开格式校验脚本和公开 smoke test；你的主要工作是提升 `main.py` 中核心算法接口的质量，而不是重新设计工程目录或输出协议。
